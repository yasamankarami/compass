...
def process_graphs(param_space, distance_cutoffs=None):
    ...
    if distance_cutoffs is None:
        # distance_cutoffs = [4.5, 5, 8, 10, 12]  # can also be provided as a list
        distance_cutoffs = [5]
    ...
    for filename in os.listdir(results_dir):
        if filename.endswith('.json') and filename.startswith('graph_cutoff_'):
            ...
            network_parameters.save_paths_and_create_heatmap(
                ...
                heatmap_file=os.path.join(results_dir,f"{prefix}_shortest_paths_heatmap.png"),
                # Heatmap file path
...
def generate_pymol_scripts(results_dir, pdb_file):
    ...
    for filename in os.listdir(results_dir):
        if filename.endswith('.json') and filename.startswith('graph_cutoff_'):
            ...
            if os.path.exists(file_paths["communities_file"]):
                visualizer.communities_pml(file_paths["communities_file"],
                                           file_paths["output_pml_communities"])

            if os.path.exists(file_paths["cliques_file"]):
                visualizer.cliques_pml(atom_mapping, file_paths["cliques_file"],
                                       file_paths["edge_betweenness_file"],
                                       file_paths["output_pml_cliques"])

            if os.path.exists(file_paths["centrality_file"]) and os.path.exists(
                    file_paths["edge_betweenness_file"]):
                visualizer.graph_pml(atom_mapping, file_paths["centrality_file"],
                                     file_paths["edge_betweenness_file"],
                                     os.path.join(results_dir,
                                                  f"{prefix}_graph"))

            if os.path.exists(file_paths["top_nodes_file"]):
                print(
                    f"Processing: {pdb_file}, {file_paths['top_nodes_file']}, {file_paths['output_pml_file']}")
                visualizer.highlight_top_nodes_pml(pdb_file, atom_mapping,
                                                   file_paths["top_nodes_file"],
                                                   file_paths["output_pml_file"])

            if os.path.exists(file_paths["paths_file"]):
                with open(file_paths["paths_file"], 'r') as f:
            ...
            visualizer.write_pml_script_for_top_shortest_paths(file_paths["top_15_file"],
                                                               file_paths["edge_betweenness_file"],
                                                               file_paths["output_top_paths"])
