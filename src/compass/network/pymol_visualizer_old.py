...
class PyMOLVisualizer:
    ...
    def write_pml_script_for_top_shortest_paths(self, top_50_file, edge_betweenness_file, output_pml_file):
        ...
        """
        edge_betweenness = {}
        with open(edge_betweenness_file, 'r') as edge_f:
            for line in edge_f:
                parts = line.strip().split(",")
                if len(parts) < 3:
                    continue
                res1 = int(parts[0].strip())
                chain1 = parts[0].strip()
                res2 = int(parts[1].strip().replace("-", ""))
                value = float(parts[2].strip())
                edge_betweenness[(res1, res2)] = value*10
                #print(value*100, res1, res2)
        # Load edge betweenness values
        #edge_betweenness = read_edge_betweenness_file(edge_betweenness_file)
                ...
                for path in paths:
                    ...
                    for i in range(0,len(path)-1):
                        ...
                        if betweenness_value is not None:
                            ...
                            # Generate sphere display command for each residue only if not already written
                            ...
                            '''
                            sphere_cmd = (
                                f"show spheres, resi {res_num1} and chain {chain_id1} and name {atom_name1} or "
                                f"resi {res_num2} and chain {chain_id2} and name {atom_name2}"
                            )
