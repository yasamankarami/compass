...
# and `pip install compass` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "compass", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n compass -f environment.docker.yml
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "compass", "YOUR_CONSOLE_SCRIPT"]
